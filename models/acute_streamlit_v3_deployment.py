...
def merge_close_points(df_points_interest, R):
    ...
    for i in range(len(centroids)):
        ...
        # print("*****************************************")
        # df_test = df_points_interest[df_points_interest['name'].str.len() > 2]
        # print(df_test)
        # print("-----------------------------------------")
...
def df_points_of_interest(df_alt_filter, R, selected_tags, api, api_key):
    ...
    if api=="OSMNX":
        ...
        for tag in tags_dict:
            for elem in tags_dict[tag]:
                ...
                df_temp = pd.DataFrame(merge_close_points(df_points_interest[cond_i], R), columns=["latitude", "longitude"])
                df_temp["class"] = tag
                df_temp["type"] = elem
                #df_temp[tag] = elem
        ...
        for ind, i in enumerate(place_result_list):
            ...
            if class_i not in tags_dict:
                tags_dict[class_i] = []
            tags_dict[class_i].append(type_i)
                # df_points_reduced["class"] = "none"
                # df_points_reduced["type"] = "none"
                # tags_dict["none"] = ["none",]
        
        # for index, row in dataox.iterrows():
        #     # If the class is not in the result dictionary, add it with an empty list
        #     if row['class'] not in tags_dict:
        #         tags_dict[row['class']] = []
        #     # Append the type to the list corresponding to the class
        #     if row["type"] not in tags_dict[row["class"]]:
        #         tags_dict[row['class']].append(row['type'])
    
    
    return df_points_reduced, tags_dict
...
def plot_point_of_interest(df, altitude_limit, min_samples, X, R, api, show_unclustered, selected_tags, token, api_key):
    ...
    df_points_interest, tags_dict = df_points_of_interest(df_alt_filter, R, selected_tags, api, api_key)
    ...
    #df_points_interest = pd.DataFrame(filtered_centroids, columns=["latitude", "longitude"])
    #df_points_interest["building"] = "university" #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    
    #show_graph_table(df_alt_filter, df_points_interest , tags_dict, parameters_dict)
    show_graph_table(df_alt_filter, df_points_interest.iloc[large_clusters], tags_dict, parameters_dict, token)

def plot_dbscan(df, altitude_limit, min_samples, eps, R, algorithm, api, show_unclustered, selected_tags, token, api_key):  
    alt_filter = (df["altitude"] > altitude_limit)
    df_alt_filter = df.loc[alt_filter].copy()
    df_points_interest, tags_dict = df_points_of_interest(df_alt_filter, R, selected_tags, api, api_key)
    ...
    show_graph_table(df_alt_filter, df_points_interest, tags_dict, parameters_dict, token)

def show_graph_table(df_alt_filter, df_points_interest, tags_dict, parameters_dict, token):
...
class FlightClusterApp:
    ...
    def display_ui(self):
        ...
        self.api = st.radio(
            "API Selection",
            ["GOOGLE", "OSMNX"],
            help="API to determine the Points of Interest"
        )
    ...
    def process_data(self):
        ...
        self.df['distance'] = self.df.apply(calculate_distance, axis=1)
    ...
    def run(self):
        ...
        if st.button('Plot Clusters'):
            if self.df_plot.empty:
                ...
                if self.algorithm == "POI":
                    plot_point_of_interest(self.df_plot, self.altitude_limit, self.min_samples, self.X, self.R, self.api, self.show_unclustered, self.selected_tags, self.token, self.api_key)
                else:
                    plot_dbscan(self.df_plot, self.altitude_limit, self.min_samples, self.eps, self.R, self.algorithm, self.api, self.show_unclustered, self.selected_tags, self.token, self.api_key)
