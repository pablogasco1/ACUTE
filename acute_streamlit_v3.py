...
def df_points_of_interest(df_alt_filter, R, selected_tags, api="OSMNX"):
...
def plot_point_of_interest(df, altitude_limit, min_samples, X, R, api, show_unclustered, selected_tags):
    ...
    df_points_interest, tags_dict = df_points_of_interest(df_alt_filter, R, selected_tags, api)
...
def plot_dbscan(df, altitude_limit, min_samples, eps, R, algorithm, api, show_unclustered, selected_tags):  
    alt_filter = (df["altitude"] > altitude_limit)
    df_alt_filter = df.loc[alt_filter].copy()
    df_points_interest, tags_dict = df_points_of_interest(df_alt_filter, R, selected_tags, api)
    ...
    def run(self):
        ...
        if st.button('Plot Clusters'):
            if self.df_plot.empty:
                ...
                if self.algorithm == "POI":
                    plot_point_of_interest(self.df_plot, self.altitude_limit, self.min_samples, self.X, self.R, self.api, self.show_unclustered, self.selected_tags)
                else:
                    plot_dbscan(self.df_plot, self.altitude_limit, self.min_samples, self.eps, self.R, self.algorithm, self.api, self.show_unclustered, self.selected_tags)
