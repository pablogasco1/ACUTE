...
def df_points_of_interest(df_alt_filter, R, selected_tags):
...
def plot_point_of_interest(df, altitude_limit, min_samples, X, R, show_unclustered, selected_tags):
    ...
    df_points_interest, tags_dict = df_points_of_interest(df_alt_filter, R, selected_tags)
...
def plot_dbscan(df, altitude_limit, min_samples, eps, R, algorithm, show_unclustered, selected_tags):  
    alt_filter = (df["altitude"] > altitude_limit)
    df_alt_filter = df.loc[alt_filter].copy()
    df_points_interest, tags_dict = df_points_of_interest(df_alt_filter, R, selected_tags)
...
class FlightClusterApp:
    ...
    def run(self):
        ...
        if st.button('Plot Clusters'):
            if self.algorithm == "POI":
                plot_point_of_interest(self.df_plot, self.altitude_limit, self.min_samples, self.X, self.R, self.show_unclustered, self.selected_tags)
            else:
                plot_dbscan(self.df_plot, self.altitude_limit, self.min_samples, self.eps, self.R, self.algorithm, self.show_unclustered, self.selected_tags)
...
# R = st.slider('Centroid Radius', min_value=0.0, max_value=0.1, value=0.02, step=0.001, format="%f", 
#                   help="Points of Interest inside the same radius will be averaged into one")
...
# if st.button('Plot Clusters'):
#     if algorithm=="POI":
#         plot_point_of_interest(df_plot, altitude_limit, min_samples, X, R, show_unclustered, selected_tags)
#     else:
#         plot_dbscan(df_plot, altitude_limit, min_samples, eps, R, algorithm, show_unclustered, selected_tags )
